...
class SimpleLingFusion(nn.Module):
    """ Fuse instruction and objects"""

    def __init__(self, output_dim, cfg, device, preprocess):
        super(SimpleLingFusion, self).__init__()
    ...
    def _build_decoder(self):
        ...
        self.lat_fusion = FusionMult(input_dim=1024)
    ...
    def forward(self, obj, goal):
        ...
        o_enc, o_emb, o_mask = self.encode_text(obj)
        g_enc, g_emb, g_mask = self.encode_text(goal)
        o_input = o_enc # l_emb if 'word' in self.lang_fusion_type else l_enc
        o_input = o_input.to(dtype=torch.float32)
        g_input = g_enc # l_emb if 'word' in self.lang_fusion_type else l_enc
        g_input = g_input.to(dtype=torch.float32)
        ...
        # print('afa', o_input.shape)

        o_input = self.conv1(o_input)
        o_input = F.leaky_relu(o_input)
        g_input = self.conv2(g_input)
        g_input = F.leaky_relu(g_input)
        x = self.lat_fusion(o_input, g_input)
